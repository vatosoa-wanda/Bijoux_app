...
def _f(name: str, field_type: str) -> bigquery.SchemaField:
    """Raccourci : tous les champs de la zone raw sont NULLABLE.

    On ne connaît pas encore, à ce stade du pipeline, si une valeur sera
    toujours renseignée dans PostgreSQL — la zone raw doit rester tolérante.
    """
...
class LoadSpec:
    ...
    schema: list[bigquery.SchemaField]       # schéma explicite (pas d'auto-détection)


# Schéma explicite par table : évite que BigQuery devine mal le type d'une
# colonne quand, par hasard, toutes ses valeurs sont NULL dans le batch du
# jour (cas vécu sur prix_total, id_commande, prix_vente...). Les types
# reprennent ceux du schéma PostgreSQL source (base_de_donnees.sql).
SCHEMA_MOUVEMENT_STOCK = [
    _f("id_mouvement", "INTEGER"),
    _f("id_matiere", "INTEGER"),
    _f("matiere_nom", "STRING"),
    _f("id_categorie", "INTEGER"),
    _f("id_type_mvt", "INTEGER"),
    _f("type_mouvement_code", "STRING"),
    _f("sens", "STRING"),
    _f("quantite", "NUMERIC"),
    _f("prix_total", "NUMERIC"),
    _f("reference_externe", "STRING"),
    _f("commentaire", "STRING"),
    _f("date_mouvement", "TIMESTAMP"),
    _f("date_extraction", "TIMESTAMP"),
]

SCHEMA_ORDRE_FABRICATION = [
    _f("id_of", "INTEGER"),
    _f("id_bijou", "INTEGER"),
    _f("bijou_nom", "STRING"),
    _f("bijou_reference", "STRING"),
    _f("id_statut_prod", "INTEGER"),
    _f("statut_code", "STRING"),
    _f("statut_libelle", "STRING"),
    _f("id_commande", "INTEGER"),
    _f("reference", "STRING"),
    _f("quantite_prevue", "INTEGER"),
    _f("quantite_realisee", "INTEGER"),
    _f("quantite_rejetee", "INTEGER"),
    _f("date_debut", "DATE"),
    _f("date_fin_prevue", "DATE"),
    _f("date_fin_reelle", "DATE"),
    _f("date_extraction", "TIMESTAMP"),
]

SCHEMA_CONTROLE_QUALITE = [
    _f("id_controle", "INTEGER"),
    _f("id_of", "INTEGER"),
    _f("date_controle", "TIMESTAMP"),
    _f("quantite_controlee", "INTEGER"),
    _f("quantite_validee", "INTEGER"),
    _f("quantite_rejetee", "INTEGER"),
    _f("commentaire_controle", "STRING"),
    _f("id_defaut", "INTEGER"),
    _f("id_type_defaut", "INTEGER"),
    _f("defaut_code", "STRING"),
    _f("defaut_libelle", "STRING"),
    _f("quantite_defaut", "INTEGER"),
    _f("commentaire_defaut", "STRING"),
    _f("date_extraction", "TIMESTAMP"),
]

SCHEMA_PRODUIT_FINI = [
    _f("id_produit_fini", "INTEGER"),
    _f("id_bijou", "INTEGER"),
    _f("id_of", "INTEGER"),
    _f("reference", "STRING"),
    _f("quantite_stock", "INTEGER"),
    _f("cout_revient", "NUMERIC"),
    _f("prix_vente", "NUMERIC"),
    _f("date_fabrication", "DATE"),
    _f("statut", "STRING"),
...
SCHEMA_BIJOU = [
    _f("id_bijou", "INTEGER"),
    _f("id_type_bijou", "INTEGER"),
    _f("id_collection", "INTEGER"),
    _f("reference", "STRING"),
    _f("nom", "STRING"),
    _f("taille", "STRING"),
    _f("complexite", "STRING"),
    _f("temps_fabrication_minutes", "INTEGER"),
    _f("photo_url", "STRING"),
...
LOAD_SPECS: list[LoadSpec] = [
    LoadSpec("mouvement_stock", "raw_mouvement_stock", SCHEMA_MOUVEMENT_STOCK),
    LoadSpec("ordre_fabrication", "raw_ordre_fabrication", SCHEMA_ORDRE_FABRICATION),
    LoadSpec("controle_qualite", "raw_controle_qualite", SCHEMA_CONTROLE_QUALITE),
    LoadSpec("produit_fini", "raw_produit_fini", SCHEMA_PRODUIT_FINI),
    LoadSpec("bijou", "raw_bijou", SCHEMA_BIJOU),
    LoadSpec("collection", "raw_collection", SCHEMA_COLLECTION),
]
...
def _cast_numeric_columns(df: pd.DataFrame, schema: list[bigquery.SchemaField]) -> pd.DataFrame:
    """Convertit en `Decimal` les colonnes déclarées NUMERIC dans le schéma.

    pyarrow ne sait pas convertir un float64 pandas directement en
    decimal128 (type interne de NUMERIC) : la conversion "brute" échoue
    avec `ArrowInvalid: Got bytestring of length 8 (expected 16)`, un
    problème connu entre certaines versions de pyarrow et
    google-cloud-bigquery. Passer par de vrais objets Decimal côté pandas
    contourne ce chemin de conversion cassé.
    """
    numeric_columns = [f.name for f in schema if f.field_type == "NUMERIC"]
